...
h = np.linspace(0,T*2*np.pi, len(d[0,:]));
agm = gm0 + (a0**2*np.sin(h)**2/2 + a0*b0y/(1-b0x)*np.sin(h))*rs;
...
if opts['--velocity']:
    ...
    y = d[3+select[1],:];
    ax = 1 - (1-b0x)*gm0/agm;
    ay = (gm0*b0y + a0*np.sin(h))/agm;
    az = (gm0*b0z)/agm;
elif opts['--momentum']:
    labels = [r'$\gamma\beta_'+l for l in ['x$','y$','z$']];
    x = d[3+select[0],:];
    y = d[3+select[1],:];
    gm = 1/np.sqrt(1-d[3,:]**2-d[4,:]**2-d[5,:]**2);
    x*=gm;
    y*=gm;
    ax = agm - gm0*(1-b0x);
    #ax = gm0*b0x + (a0**2*np.sin(h)**2/2 + a0*b0y/(1-b0x)*np.sin(h))*rs;
    ay = gm0*b0y + a0*np.sin(h);
    az = gm0*b0z;
else:
    labels = ['$x$','$y$','$z$'];
    x = d[select[0],:];
    y = d[select[1],:];
    ax = h*b0x/(1-b0x) + a0**2/4*rs**2*(h - np.sin(2*h)/2) + a0*(np.cos(h)-1)*rs*b0y/(1-b0x);
    ay = h*b0y/(1-b0x) + a0*rs*(np.cos(h)-1);
    az = h*b0z/(1-b0x);
...
plt.plot(ay, ax, marker='+',label='analytic',c='r');
